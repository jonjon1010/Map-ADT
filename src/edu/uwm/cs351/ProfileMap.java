// worked with Alex Seinfeld on the concepts for this homework for the invariant
...
public class ProfileMap {
	...
	private boolean wellFormed() {
		...
		//1 
		if(!Primes.isPrime(table.length - 2) || !Primes.isPrime(table.length)) return report("one or both of the twin primes are not prime numbers!");
		if(table.length < table.length - 2) return report("table size is not the larger of the two twin primes!");
		
		//2 
		for(int i = 0; i < table.length; i++) {
			if(table[i] != null && table[i].key != null && table[i] != PLACE_HOLDER) {
				int index = getEntryIndex(table[i].key,false);
				if(i != index) return report("profile is not at the correct index!");
				for(int j = index; j < table.length; j++) {
					if(j != index && table[j] != null && table[j] != PLACE_HOLDER && table[index].key.equals(table[j].key)) return report("");
				}
			}
		}
		...
		for(int i = 0; i < table.length; i++) {
			if(table[i] != null) {
				countUsed++;
				if(table[i].key != null) countEntries++;
				// 3 
				if(table[i].key == null && table[i] != PLACE_HOLDER) return report("");
			}	
		}
		if(used != countUsed) return report("number of non-null entries is " + countUsed + " ,while used is " + used + "!");
		if(numEntries != countEntries) return report("number of non-null, non Placed_Holder entries is " + countEntries + " ,while numEntries is " + numEntries + "!");
		
		//6
		if(((double)used/(double)table.length) >= CROWDED) return report("");
	...
	private int getEntryIndex(Profile x, boolean nullKeyOK) {
		...
		int hash1 = Math.abs(x.hashCode()) % table.length;
		int hash2 = 1 + (Math.abs(x.hashCode())) % (table.length - 2);
		int index = hash1; 
		if(nullKeyOK){
			...
			if(table[index] != null && table[index].key != x) {
				int i = index;
				while(table[i] != null) {
					i = index = (index + hash2) % table.length;
					if(table[i] != null && table[i] != PLACE_HOLDER && table[i].key.equals(x)) return index;
				}
			}
		}
		return index;
	...
	private void rehash() {
		...
		Entry[] temp = new Entry[Primes.nextTwinPrime(4*numEntries)];
		for(int i = 0; i < table.length; i++) {
			if(table[i] != null) {
				if(table[i] != PLACE_HOLDER) temp[i] = table[i];
			}
		}
		table = temp;
	}
	...
	public int size() {
		...
		return used; // TODO: very little work
	}
	...
	public boolean add(Profile p1, Profile p2) {
		...
		//TODO: implement this
		if(find(p1) != null) return result;
		numEntries++;
		used++;
		if(((double)used/(double)table.length) >= CROWDED) rehash();
		int index = getEntryIndex(p1, true);
		table[index] = new Entry(p1);
	...
	public ArrayList<Profile> find(Profile key) {
		...
		for(int i = 0; i < table.length; i++) {
			if(table[i] != null && table[i] != PLACE_HOLDER && table[i].key.equals(key)) return table[i]; 
		}
		return null; // TODO
	}
	...
	public ArrayList<Profile> getAll() {
		...
		for(int i = 0; i < table.length; i++) {
			if(table[i] != null) allProfiles.add(table[i].key);
		}
		return allProfiles;
	}
	...
	public ArrayList<Profile> remove(Profile key) {
		...
		if(find(key) == null) return getAll();
		else {
			for(int i = 0; i < table.length; i++) {
				if(table[i] != null && table[i] != PLACE_HOLDER && table[i].key.equals(key)) {
					result = getAll();
					table[i] = PLACE_HOLDER;
					numEntries--;
					break;
				}
			}
